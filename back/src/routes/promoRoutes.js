...
router.get("/", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch promotions. " + error.message });
  }
});
...
router.post("/promos-with-availability", (req, res) => {
  try {
    ...
    if (
      !["price_discount", "percentage_discount", "buy_x_get_y"].includes(
        promoData.type
      )
    ) {
      return res
        .status(400)
        .json({
          error: "Invalid promo type. Must be 'discount' or 'buy_x_get_y'.",
        });
    }

    // Ensure dates are properly formatted
    promoData.startDate = promoData.startDate
      ? new Date(promoData.startDate).toISOString()
      : null;
    promoData.endDate = promoData.endDate
      ? new Date(promoData.endDate).toISOString()
      : null;
      ...
      recurrentDays.forEach(([day, times]) => {
        if (times && times.startTime && times.endTime) {
          const recurrentDate = new RecurrentDate({
            promoId: id,
            dayOfWeek: day.charAt(0).toUpperCase() + day.slice(1), // Capitalize day
    ...
    res
      .status(201)
      .json({ id, message: "Promo and availability created successfully." });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({
        error: `Failed to create promo and availability. ${error.message}`,
      });
  }
});
...
router.put("/promos-with-availability/:id", async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        id: promo.id,
        message: "Promo and availability updated successfully.",
      });
  } catch (error) {
    console.error(error);
    res
      .status(500)
      .json({
        error: `Failed to update promo and availability. ${error.message}`,
      });
  }
});
...
router.get("/promos-with-availability", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch promotions with availability." });
  }
});
...
router.get("/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch promotion. " + error.message });
  }
});
...
router.post("/", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create promotion. " + error.message });
  }
});
...
router.put("/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update promotion. " + error.message });
  }
});
...
router.delete("/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete promotion. " + error.message });
  }
});
...
router.get("/:id/recurrence-rules", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch recurrence rules. " + error.message });
  }
});
...
router.post("/:id/recurrence-rules", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create recurrence rule. " + error.message });
  }
});
...
router.put("/recurrence-rules/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update recurrence rule. " + error.message });
  }
});
...
router.delete("/promos/recurrence-rules/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete recurrence rule. " + error.message });
  }
});
...
router.get("/recurrent-dates", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch recurrent dates. " + error.message });
  }
});
...
router.get("/recurrent-dates/promo/:promoId", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch recurrent dates for promo. " + error.message,
      });
  }
});
...
router.post("/recurrent-dates", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create recurrent date. " + error.message });
  }
});
...
router.put("/recurrent-dates/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to update recurrent date. " + error.message });
  }
});
...
router.delete("/recurrent-dates/:id", (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to delete recurrent date. " + error.message });
  }
});
