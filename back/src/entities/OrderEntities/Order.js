...
class Order {
    ...
    constructor({ id, customer, subtotal, discountTotal, total, tip, createdAt, paymentMethod, cancelledAt, cancelReason, status, claimedById, billedById }) {
        this.id = id || null;
    ...
    static getByCustomer(customer) {
        const stmt = db.prepare(`SELECT * FROM ${this.tableName} WHERE customer = ?`);
